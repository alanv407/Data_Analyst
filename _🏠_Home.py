...
with st.container(border=True):
    ...
    st.markdown('In some cases you will find a box like the one shown below, by clicking you will see more information')
...
with st.container(border=True):
    st.subheader('Tabs')
    st.markdown('In other cases you will find Tabs, to see the information of each one you just have to click on each one')
...
with st.container(border=True):    
    st.markdown('**You can find the code for this web application on GitHub:** https://github.com/alanv407/Data_Analyst')
